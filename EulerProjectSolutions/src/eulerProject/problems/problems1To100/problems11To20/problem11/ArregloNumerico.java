...
public class ArregloNumerico {
	private int[][] arr;
	private int[] arrMultMax;
	private int largoMax;
	private int multMaxima;
	private String direccion;

	public ArregloNumerico(int largoMax) {
		llenarArreglo();
		
		this.largoMax = largoMax;
		this.arrMultMax = new int[largoMax];
		this.multMaxima = 0;
	...
	private void buscarMultMaxima() {
		int[] miniArr = new int[largoMax];

		// busqueda horizontal
		busquedaHorizontal(miniArr);
		
		// busqueda vertical
		busquedaVertical(miniArr);
		
		// busqueda diagonal hacia abajo
		busquedaDiagonalAbajo(miniArr);
		
		// busqueda diagonal hacia arriba
		busquedaDiagonalArriba(miniArr);
	}

	public int[] getArrMultMax() {
		return arrMultMax;
	}

	public int getMultMaxima() {
		return multMaxima;
	}
	...
	private int multMiniArr(int[] miniArr) {
		int resultado = 1;
	...
	private void busquedaHorizontal(int[] miniArr) {
		// busqueda horizontal
		for (int filas = 0; filas < 20; filas++) {
			for (int columnas = 0; columnas < 20 - largoMax + 1; columnas++) {
				for (int idx = 0; idx < largoMax; idx++) {
					miniArr[idx] = arr[filas][columnas + idx];
				}

				int sum = multMiniArr(miniArr);
	...
	private void busquedaVertical(int[] miniArr) {
		for (int filas = 0; filas < 20 - largoMax + 1; filas++) {
			for (int columnas = 0; columnas < 20; columnas++) {
				for (int idx = 0; idx < largoMax; idx++) {
					miniArr[idx] = arr[filas + idx][columnas];
				}
	...
	private void busquedaDiagonalAbajo(int[] miniArr) {
		for (int filas = 0; filas < 20 - largoMax + 1; filas++) {
			for (int columnas = 0; columnas < 20 - largoMax + 1; columnas++) {
				for (int idx = 0; idx < largoMax; idx++) {
					miniArr[idx] = arr[filas + idx][columnas + idx];
				}
				...
				if (sum > multMaxima) {
					multMaxima = sum;
					arrMultMax = miniArr;
					direccion = "DIAG_ABAJO";
				}
			}
		}
	}
	
	private void busquedaDiagonalArriba(int[] miniArr) {
		for (int filas = 0; filas < 20 - largoMax + 1; filas++) {
			for (int columnas = 0; columnas < 20 - largoMax + 1; columnas++) {
				for (int idx = 0; idx < largoMax; idx++) {
					miniArr[idx] = arr[filas + largoMax - 1 - idx][columnas + idx];
				}
